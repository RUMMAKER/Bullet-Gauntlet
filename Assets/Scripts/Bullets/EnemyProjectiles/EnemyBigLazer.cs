...
public class EnemyBigLazer : MonoBehaviour {
	...
	void DamagePlayer(){
		RaycastHit2D[] hits = Physics2D.BoxCastAll(transform.position, new Vector2(finalWidth, finalWidth), transform.rotation.eulerAngles.z, new Vector2(Mathf.Cos(transform.rotation.eulerAngles.z*Mathf.Deg2Rad), Mathf.Sin(transform.rotation.eulerAngles.z*Mathf.Deg2Rad)), 25f, 1<<10, -10f, 10f);
		foreach(RaycastHit2D hit in hits){
			if(hit.collider != null){
				Ship s = hit.collider.gameObject.GetComponent<Ship>();
				if(s != null){
					s.TakeDamage(damage*Time.deltaTime);
				}
			}
		}
	}
